...
def model_load(prefix='sl',data_dir=None,training=True):
    ...
    if not data_dir:
        ...
        print(f"***{data_dir}")
    models = [f for f in os.listdir(os.path.join(os.path.dirname(data_dir),"models")) if re.search("sl",f)]
    print()
    if len(models) == 0:
    ...
    all_models = {}
    for model in models:
        all_models[re.split("-",model)[1]] = joblib.load(os.path.join(os.path.dirname(__file__),"models",model))
...
def model_predict(country,year,month,day,all_models=None,test=False):
    ...
    #print(f"*********first all models {all_models}")
    if not all_models:
