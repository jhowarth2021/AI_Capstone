...
def engineer_features(df,training=True):
    ...
    ## extract dates - deep copy
    dates = df['date'].values.copy()
    ## convert dates to datetime type
    dates = dates.astype('datetime64[D]')

    ## engineer some features
    ##default dict allows for creating key-value pair if key not already in
    eng_features = defaultdict(list)
    
    previous =[7, 14, 28, 70]  #[7, 14, 21, 28, 35, 42, 49, 56, 63, 70]
    
    #array of zero values the length of number of dates present
    ...
    for d,day in enumerate(dates):
        ...
        ##for each of the timeframes (7, 14, etc)

        for num in previous:
            current = np.datetime64(day, 'D')
            ##calculate the prev day using a time delta
            prev = current - np.timedelta64(num, 'D')
            ##create a mask by doing element-wsie comparison
            ##compares all dates to array of prev to current day
            ##in effect this should only grab those days in that span
            mask = np.in1d(dates, np.arange(prev,current,dtype='datetime64[D]'))
            ##adds the total revenue from the time window to the prev_{} key
            ##note, because its a defaultdict it creates the value {} if no
            ##key is present
            eng_features["previous_{}".format(num)].append(df[mask]['revenue'].sum())

        ## get get the target revenue  similar process but single 30 day value
        ## used as target
    ...
    #grabs days which had some information
    mask = X.sum(axis=1)>0
    X = X[mask]
    y = y[mask]
    dates = dates[mask]
...
if __name__ == "__main__":
    ...
    data_dir = os.path.join(os.path.dirname(__file__),"cs-train")
    print("...fetching data")
    
    #grab dictionary of dataframes
